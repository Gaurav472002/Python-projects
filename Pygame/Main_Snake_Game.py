...
def welcome(): # function for the welcome screen
    exit_game = False
    while not exit_game:
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT: # to quit the game using the cross button on the top
                    exit_game = True
            ...
            #movements for the snake associated with the required keys
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    exit_game = True
            ...
            pygame.draw.rect(gameWindow, red, [food_x, food_y, snake_size, snake_size]) # plotting the food on the screen
            ...
            # game over if the head of the snake crashes in its body
            if head in snk_list[:-1]:
                game_over = True
                pygame.mixer.music.load('gameover.mp3')
                pygame.mixer.music.play()
            
            # game over if the head of the snake crashes to the outer boundaries
